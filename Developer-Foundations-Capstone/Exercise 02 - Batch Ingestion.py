...
#3 Using the information in fixed_width_column_defs (or the dictionary itself) use the value column to extract each new column of the appropriate length.
df=df.select(
...
for column in df.columns:
  df = df.withColumn(column, F.ltrim(df[column]))
...
for column in df1.columns:
  df1 = df1.withColumn(column, when(col(column) == 'null', None).otherwise(col(column)))
df_concat = df.union(df1)
print((df_concat.count(), len(df_concat.columns)))
df_concat.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save(batch_target_path)
...
column_names =["submitted_at","order_id","customer_id","sales_rep_id","sales_rep_ssn","sales_rep_first_name","sales_rep_last_name","sales_rep_address","sales_rep_city","sales_rep_state","sales_rep_zip","shipping_address_attention","shipping_address_address","shipping_address_city","shipping_address_state","shipping_address_zip","product_id","product_quantity","product_sold_price"]
df2 = df2.toDF(*column_names)

#3 Add a new column, ingest_file_name, which is the name of the file from which the data was read from - note this should not be hard coded.
df2=df2.withColumn("ingest_file_name",F.input_file_name())
#4    Add a new column, ingested_at, which is a timestamp of when the data was ingested as a DataFrame - note this should not be hard coded.
df2=df2.withColumn('ingested_at', F.current_timestamp())
#5 Append the corresponding DataFrame to the previously created dataset specified by batch_target_path
for column in df2.columns:
  df2 = df2.withColumn(column, when(col(column) == 'null', None).otherwise(col(column)))

df_concat2 = df2.union(df_concat)
print((df_concat2.count(), len(df_concat2.columns)))
df_concat2.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save(batch_target_path)
