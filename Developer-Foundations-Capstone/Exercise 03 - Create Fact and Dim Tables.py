...
# MAGIC %sql
# MAGIC CREATE DATABASE if not exists dbacademy_tamas_kusnyerik_accenture_com_developer_foundations_capstone;
# MAGIC USE dbacademy_tamas_kusnyerik_accenture_com_developer_foundations_capstone;
...
# MAGIC %scala
# MAGIC spark.sqlContext.cacheTable("batched_orders") 
...
#df.select('_error_ssn_format','sales_rep_ssn').filter(df._error_ssn_format == True).show()

# COMMAND ----------

df = df.withColumn('sales_rep_ssn', regexp_replace('sales_rep_ssn', '-', ''))
df = df.withColumn("sales_rep_ssn", df["sales_rep_ssn"].cast("bigint"))

df = df.withColumn("sales_rep_zip", df["sales_rep_zip"].cast("int"))

drop_list=['submitted_at','order_id','customer_id','shipping_address_attention','shipping_address_address','shipping_address_city','shipping_address_state','shipping_address_zip','product_id','product_quantity','product_sold_price']
df=df.drop(*drop_list)

df=df.drop_duplicates(subset=['sales_rep_id','sales_rep_ssn','sales_rep_first_name','sales_rep_last_name','sales_rep_address','sales_rep_city','sales_rep_state','sales_rep_zip'])
...
df.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("sales_reps")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from sales_reps;
...
df2 = spark.read.format("delta").load(batch_source_path)

# COMMAND ----------

from pyspark.sql import functions as F
#convert to timestamp
df2 = df2.withColumn("submitted_at",F.from_unixtime(df2["submitted_at"]))
df2 = df2.withColumn("submitted_at",F.to_timestamp(df2["submitted_at"]))
#cast to int
df2 = df2.withColumn("shipping_address_zip", df2["shipping_address_zip"].cast("int"))
#drop unnecessary columns
drop_list=['product_id','product_quantity','product_sold_price','sales_rep_ssn','sales_rep_first_name','sales_rep_last_name','sales_rep_address','sales_rep_city','sales_rep_state','sales_rep_zip']
df2=df2.drop(*drop_list)
#drop duplicates
df2=df2.drop_duplicates(subset=['submitted_at','order_id','customer_id','shipping_address_attention','shipping_address_address','shipping_address_city','shipping_address_state','shipping_address_zip'])
#format date
df2=df2.withColumn("submitted_yyyy_mm",  F.date_format(F.col("submitted_at"), "yyyy-MM"))
#write partitioned table 
df2.write.partitionBy("submitted_yyyy_mm").mode("overwrite").option("overwriteSchema", "true").saveAsTable("orders")

# COMMAND ----------

# MAGIC %sql
# MAGIC show partitions orders
...
df1 = spark.read.format("delta").load(batch_source_path)

# COMMAND ----------

df1 = df1.select(df1.order_id,df1.product_id,df1.product_quantity,df1.product_sold_price,df1.ingest_file_name,df1.ingested_at)

# COMMAND ----------

df1 = df1.withColumn("product_quantity", df1["product_quantity"].cast("int"))
df1 = df1.withColumn("product_sold_price", df1["product_sold_price"].cast("decimal(10,2)"))
df1.printSchema()

# COMMAND ----------

df1.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("line_items")
