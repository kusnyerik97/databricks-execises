...
# MAGIC %sql
# MAGIC create or replace temporary view question_1_results  AS select shipping_address_state,count(order_id) as count from orders group by shipping_address_state order by count DESC;
...
# MAGIC %sql
# MAGIC select * from line_items 

# COMMAND ----------

# MAGIC %sql
# MAGIC select avg(product_sold_price),min(product_sold_price),max(product_sold_price) from line_items inner join orders on orders.order_id=line_items.order_id where orders.shipping_address_state="NC" group by orders.shipping_address_state;
...
# MAGIC SELECT *
# MAGIC FROM orders
# MAGIC INNER JOIN line_items ON orders.order_id=line_items.order_id
# MAGIC INNER JOIN products  ON products.product_id=line_items.product_id
# MAGIC INNER JOIN sales_reps  ON sales_reps.sales_rep_id=orders.sales_rep_id
# MAGIC where products.color="green" and orders.shipping_address_state="NC" and sales_reps._error_ssn_format=TRUE;# TODO
# MAGIC 
# MAGIC select avg(product_sold_price) * from products;
# MAGIC 
# MAGIC 
# MAGIC ex_avg = 0 # FILL_IN
# MAGIC ex_min = 0 # FILL_IN
# MAGIC ex_max = 0 # FILL_IN
