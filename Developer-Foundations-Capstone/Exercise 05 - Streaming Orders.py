...
df_test = spark.read.json("dbfs:/user/tamas.kusnyerik@accenture.com/dbacademy/developer-foundations-capstone/raw/orders/stream/order_0612a18b-0cc7-43ea-9f5b-155aad967cb9_2020-01-01.json")
json_schema=df_test.schema

# COMMAND ----------

orders = spark.table("orders")
orders_schema=orders.schema
orders.printSchema()
...
##exploding array into fields
#df=df.withColumn("productId", explode(df.productId))
#df=df.withColumn("quantity", explode(df.quantity))
#df=df.withColumn("soldPrice", explode(df.soldPrice))


# Add a new column, ingest_file_name  
...
df = df.withColumn("submitted_at",F.from_unixtime(df["submittedAt"]))
df = df.withColumn("submitted_at",F.to_timestamp(df["submittedAt"]))

#convert to int
df = df.withColumn("zip", df["zip"].cast("int"))

#format date
df=df.withColumn("submitted_yyyy_mm",  F.date_format(F.col("submitted_at"), "yyyy-MM"))

#drop columns

drop_list=['productId','quantity','soldPrice','submittedAt']
...
df.writeStream.format("delta").outputMode("append").partitionBy("submitted_yyyy_mm").queryName("orders").option("checkpointLocation", orders_checkpoint_path).table("orders")
...
df1=df1.select('orderId',explode(df1.products))

#flatten column array
def flatten(schema, prefix=None):
    fields = []
    for field in schema.fields:
        name = prefix + '.' + field.name if prefix else field.name
...
df1=df1.select(flatten(df1.schema))

# Add a new column, ingest_file_name  
df1=df1.withColumn("ingest_file_name",F.input_file_name())

# Add a new column, ingested_at
df1=df1.withColumn('ingested_at', F.current_timestamp())

df1 = df1.withColumnRenamed("orderId","order_id") \
...
df1.writeStream.format("delta").outputMode("append").queryName("line_items").option("checkpointLocation", line_items_checkpoint_path).table("line_items")
