...
xmldata = spark.read.format("com.databricks.spark.xml").option("rootTag","products").option("rowTag","product").option("inferSchema",True).load(products_xml_path)
#xmldata.printSchema()
#display(xmldata)
...
df=xmldata.select(flatten(xmldata.schema))
...
df=df.withColumnRenamed("_base_price","base_price") \
...
df.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("products")
