...
outputPathSilver = f"{outputPath}/silver_test"
outputPathGold   = f"{outputPath}/gold_test"
...
def writeToBronze(sourceDataframe, bronzePath, streamName):
   (sourceDataframe
    ...
    .writeStream.option("checkpointLocation", f"{bronzePath}_checkpoint")
...
temp_gaming=spark.read.format("delta").load(outputPathBronze).select(col("eventParams.device_id")).alias("device_id")
...
def loadStaticData(path):
  return spark.read.format("delta").load(path).join(temp_gaming,"device_id")
...
def bronzeToSilver(bronzePath, silverPath, streamName, lookupDF):
  (spark.readStream
    .format("delta")
    .load(bronzePath)
    .select(col("eventParams.device_id"), col("eventName"),col("eventParams.client_event_time"),col("eventDate"))
...
from pyspark.sql.functions import weekofyear, approx_count_distinct, col, countDistinct
